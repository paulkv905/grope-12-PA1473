...
def code():
    ...
    def set_drop_off_zones():
        ev3.screen.clear()
        ev3.speaker.say("Assign 3 drop of zones. Use the arrow keys to align the crane with the drop of zone. Then press the center button to select.")
        rotation_temp_int = 0
    ...
    def idle_mode(zone_info_matrix):
        ev3.speaker.say("bravo 6 going dark")
        elevation.run_until_stalled(500, then=Stop.HOLD, duty_limit=25)
        time.sleep(10)
        ev3.speaker.say("resuming work")
    ...
    def set_zones_or_load():
        ev3.screen.clear()
        ev3.speaker.say("up to load")
        ev3.speaker.say("down for manually")
        while True:
            up_button = Button.UP in ev3.buttons.pressed()
            down_button = Button.DOWN in ev3.buttons.pressed()
            if up_button:
